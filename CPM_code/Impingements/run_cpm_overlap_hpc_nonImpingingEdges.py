...
def run_overlap_cpm(
    ...
    yhat_non_overlap = np.full((N, Nsymp, Ncog), np.nan) 
    yhat_overlap_null_non = np.full((N, Nsymp, Ncog), np.nan) 
    yhat_none = np.full((N, Nsymp, Ncog), np.nan) 
    yhat_null_none = np.full((N, Nsymp, Ncog), np.nan) 
    ...
    r_non_overlap  = np.full((Nsymp, Ncog), np.nan)
    p_non_overlap  = np.full((Nsymp, Ncog), np.nan)

    r_none   = np.full((Nsymp, Ncog), np.nan)
    p_none   = np.full((Nsymp, Ncog), np.nan)
        ...
        for s in range(Nsymp):
            ...
            for c in range(Ncog):
              ...
              '''   train_sum = np.nansum(train_devs[pos_ov, :], axis=0) - (np.nansum(train_devs[neg_ov, :], axis=0))
                ...
                ### Train/test on clinical edges that do not overlap with cognitive networks
                # edges that are clinical_pos but NOT cog_pos
                pos_non_overlap_mask = clin_pos & ~cog_pos 
                pos_non_overlap_indices = np.where(pos_non_overlap_mask)[0]

                # randomly sample the same number as pos overlapping edges
                rng = np.random.default_rng(42)   # or remove seed if you want different each time
                pos_non_ov = rng.choice(
                    pos_non_overlap_indices,
                    size=len(pos_indices),
                    replace=False
                )

                # edges that are clinical_neg but NOT cog_neg
                neg_non_overlap_mask = clin_neg & ~cog_neg
                neg_non_overlap_indices = np.where(neg_non_overlap_mask)[0]

                # randomly sample the same number as neg overlapping edges
                neg_non_ov = rng.choice(
                    neg_non_overlap_indices,
                    size=len(neg_indices),
                    replace=False
                )
                
                train_sum_non = np.nansum(train_devs[pos_non_ov, :], axis=0) - (np.nansum(train_devs[neg_non_ov, :], axis=0))
                y_train_c_non = cogdata[train_idx, c]

                m = (~np.isnan(train_sum_non)) & (~np.isnan(y_train_c_non))

                a, b = np.polyfit(train_sum_non[m], y_train_c_non[m], 1)

                # Predict on TEST
                test_sum_non = np.nansum(test_devs[pos_non_ov, :], axis=0) - (np.nansum(test_devs[neg_non_ov, :], axis=0))
                yhat_non_overlap[test_idx, s, c] = a * test_sum_non + b

                # Perform null tests
                y_train_c_null_non = shuffled_cog[train_idx, c]

                m = (~np.isnan(train_sum_non)) & (~np.isnan(y_train_c_null_non))

                a, b = np.polyfit(train_sum_non[m], y_train_c_null_non[m], 1)

                # Predict on TEST
                test_sum_non = np.nansum(test_devs[pos_non_ov, :], axis=0) - (np.sum(test_devs[neg_non_ov, :], axis=0))
                yhat_overlap_null_non[test_idx, s, c] = a * test_sum_non + b'''
                
                ### Train/test on edges that are not in clinical or cognitive networks
                none_mask = ~clin_pos & ~cog_pos & ~clin_neg & ~cog_neg
                ...
                none = rng.choice(
                    none_indices,
                    size=len(pos_indices)+len(neg_indices),
                    replace=False
                )

                
                train_sum_none = np.nansum(train_devs[none, :], axis=0) 
                y_train_c_none = cogdata[train_idx, c]

                m = (~np.isnan(train_sum_none)) & (~np.isnan(y_train_c_none))

                a, b = np.polyfit(train_sum_none[m], y_train_c_none[m], 1)

                # Predict on TEST
                test_sum_none = np.nansum(test_devs[none, :], axis=0)
                yhat_none[test_idx, s, c] = a * test_sum_none + b

                # Perform null tests
                y_train_c_null_none = shuffled_cog[train_idx, c]

                m = (~np.isnan(train_sum_none)) & (~np.isnan(y_train_c_null_none))

                a, b = np.polyfit(train_sum_none[m], y_train_c_null_none[m], 1)

                # Predict on TEST
                test_sum_none = np.nansum(test_devs[none, :], axis=0)
                yhat_null_none[test_idx, s, c] = a * test_sum_none + b

    # --- Evaluate performance across all subjects ---
    for s in range(Nsymp):
        for c in range(Ncog):
            '''r_overlap[s, c],   p_overlap[s, c]   = corr_no_nan(yhat_overlap[:, s, c], cogdata[:, c], kind="spearman")
            r_non_overlap[s, c],   p_non_overlap[s, c]   = corr_no_nan(yhat_non_overlap[:, s, c], cogdata[:, c], kind="spearman")'''
            r_none[s, c],   p_none[s, c]   = corr_no_nan(yhat_none[:, s, c], cogdata[:, c], kind="spearman")
    ...
    return {
        ...
        'r_non_overlap': r_non_overlap,
        'p_non_overlap': p_non_overlap,'''
        'r_none': r_none,
        'p_none': p_none,

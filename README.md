...
- Input data for CPM are stored here: {insert link to data on Figshare}. Be sure to update relevant filepaths in runCPM_allvars.m accordingly!
...
- Make sure that the filepaths load in the correct data. Dependency files (e.g., surface files) are included in this repository. The output data from CPM (network masks and prediction strength values) are stored here: {insert link to data on Figshare}. Be sure to update all relevant filepaths in the notebook accordingly!
